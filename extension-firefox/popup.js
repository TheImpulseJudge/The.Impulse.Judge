/**
 * Impulse Buy Judge - Popup Script v1.0 (Firefox)
...
async function saveSettings(settings) {
  try {
    await browserAPI.runtime.sendMessage({ type: 'SAVE_SETTINGS', settings });
  } catch (e) {
...
function updateAchievements(unlockedAchievements) {
  ...
  const achievementsHTML = displayEntries.map(([key, achievement]) => {
    const isUnlocked = unlockedSet.has(key);
    return `
      <div class="achievement-badge ${isUnlocked ? 'unlocked' : 'locked'}" data-key="${escapeHTML(key)}">
        <span class="achievement-icon">${escapeHTML(achievement.icon)}</span>
        <span class="achievement-name">${escapeHTML(achievement.name)}</span>
        <span class="achievement-desc">${escapeHTML(achievement.desc)}</span>
      </div>
    `;
  }).join('');
  safeSetHTML(grid, achievementsHTML);
  ...
  if (progress) {
    ...
    safeSetHTML(progress, progressHTML);
...
function updateSiteList(listType, sites) {
  ...
  if (!sites.length) {
    safeSetHTML(container, '<p class="sites-empty">No sites added</p>');
    return;
  }
  ...
  const sitesHTML = sites.map(site => `
    <div class="site-item">
      ...
      <button class="site-remove" data-site="${escapeHTML(site)}" data-list="${escapeHTML(listType)}">✕</button>
    </div>
  `).join('');
  safeSetHTML(container, sitesHTML);
...
function setupExportButton() {
  ...
  if (exportBtn) {
    exportBtn.addEventListener('click', async () => {
      try {
        const response = await browserAPI.runtime.sendMessage({ type: 'EXPORT_STATS' });
...
function updateActivityList(history) {
  ...
  const activityHTML = recentHistory.map(entry => {
    ...
    return `
      <div class="activity-item ${escapeHTML(statusClass)}">
        <span class="activity-icon">${icon}</span>
        <div class="activity-details">
          <span class="activity-product">${escapeHTML(product)}</span>
          <span class="activity-meta">${escapeHTML(priceText)} • ${escapeHTML(timeAgo)}</span>
        </div>
        <span class="activity-status">${escapeHTML(statusText)}</span>
      </div>
    `;
  }).join('');
  safeSetHTML(container, activityHTML);
}
 ...
 * Only use for trusted internal content, not user-generated content from external sources.
 */
...
function setupResetButton() {
  ...
  resetBtn.addEventListener('click', async () => {
    if (confirm('Are you sure you want to reset all statistics? This cannot be undone.')) {
      try {
        await browserAPI.runtime.sendMessage({ type: 'RESET_STATS' });
        await loadStats();
...
function setupShareButtons() {
  document.querySelectorAll('.share-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (shareUrl) {
        browserAPI.tabs.create({ url: shareUrl });
      }
    });
  });
}
